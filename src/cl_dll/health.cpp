...
int CHudHealth::Draw(float flTime)
{
	...
	else
	{
		...
		if (m_fFade)
		{
			...
			a = MIN_ALPHA_B + (m_fFade / FADE_TIME) * 200;
		}
		else
			a = MIN_ALPHA_B;
...
int CHudHealth::DrawPain(float flTime)
{
	...
	if (m_fAttackFront > 0.4)
	{
		...
		x = ScreenWidth/2 - SPR_Width(m_HLSPRITE, 0)/2;
		y = ScreenHeight/2 - SPR_Height(m_HLSPRITE,0) * 3;
	...
	if (m_fAttackRight > 0.4)
	{
		...
		x = ScreenWidth/2 + SPR_Width(m_HLSPRITE, 1) * 2;
		y = ScreenHeight/2 - SPR_Height(m_HLSPRITE,1)/2;
	...
	if (m_fAttackRear > 0.4)
	{
		...
		x = ScreenWidth/2 - SPR_Width(m_HLSPRITE, 2)/2;
		y = ScreenHeight/2 + SPR_Height(m_HLSPRITE,2) * 2;
	...
	if (m_fAttackLeft > 0.4)
	{
		...
		x = ScreenWidth/2 - SPR_Width(m_HLSPRITE, 3) * 3;
		y = ScreenHeight/2 - SPR_Height(m_HLSPRITE,3)/2;
...
int CHudHealth::DrawDamage(float flTime)
{
	...
	for (i = 0; i < NUM_DMG_TYPES; i++)
	{
		if (m_bitsDamage & giDmgFlags[i])
		{
			...
			SPR_DrawAdditive(0, pdmg->x, pdmg->y + -35, &gHUD.GetSpriteRect(m_HUD_dmg_bio + i));
		}
	}
