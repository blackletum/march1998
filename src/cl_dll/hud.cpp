...
int __MsgFunc_PunchAngle(const char* pszName, int iSize, void* pbuf) //magic nipples - elights
{
	return gHUD.MsgFunc_PunchAngle(pszName, iSize, pbuf);
}

int CHud::MsgFunc_PunchAngle(const char* pszName, int iSize, void* pbuf)
{
	BEGIN_READ(pbuf, iSize);

	gHUD.PunchAngle[0] = READ_COORD();
	gHUD.PunchAngle[1] = READ_COORD();
	gHUD.PunchAngle[2] = READ_COORD();
...
//	m_hsprFont = LoadSprite("sprites/%d_font.spr");
	...
	m_HUD_bnumber_0 = GetSpriteIndex("bnumber_0");
	m_HUD_snumber_0 = GetSpriteIndex("snumber_0");
