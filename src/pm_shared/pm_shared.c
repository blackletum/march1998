...
#define CLIMB_SHAKE_FREQUENCY	450	// how many frames in between screen shakes when climbing
#define MAX_CLIMB_SPEED			200	// fastest vertical climbing speed possible. default 200.
#define CLIMB_SPEED_DEC			0.045	// climbing deceleration rate
#define CLIMB_PUNCH_X			-7  // how far to 'punch' client X axis when climbing
#define CLIMB_PUNCH_Z			7	// how far to 'punch' client Z axis when climbing

void PM_AlphaLadderMove(physent_t* pLadder)
{
	vec3_t		ladderCenter;
	trace_t		trace;
	qboolean	onFloor;
	...
	if (trace.fraction != 1.0)
	{
		...
		if (pmove->onground != -1)
		{
			...
			{
				VectorScale(trace.plane.normal, 98, pmove->velocity);
			}
			else if (pmove->cmd.buttons & IN_FORWARD)
			{
				pmove->velocity[2] = (pmove->velocity[2] * 4) + 1;
			}
			else
			{
				pmove->velocity[0] = 0;
		...
		else
		{
			...
			{
				pmove->velocity[2] -= pmove->velocity[2] * 0.3;
				//pmove->velocity[0] = 0;
				//pmove->velocity[1] = 0;
			...
			if (forward != 0)
			{
				move_dir[0] = ladderCenter[0] - pmove->origin[0];
				move_dir[1] = ladderCenter[1] - pmove->origin[1];
				move_dir[2] = 0;

				pmove->velocity[0] = move_dir[0] * 2;
				pmove->velocity[1] = move_dir[1] * 2;

				pmove->velocity[2] -= pmove->velocity[2] * CLIMB_SPEED_DEC;
				//pmove->velocity[2] = forward;

				if (pmove->flTimeLadderPunch <= 0)
				{
					switch (pmove->leftright)
					{
					case 0:
						pmove->punchanglejit[0] = CLIMB_PUNCH_X;
						pmove->punchanglejit[2] = CLIMB_PUNCH_Z;
						pmove->leftright = 1;
						break;
					...
					pmove->flTimeLadderPunch = CLIMB_SHAKE_FREQUENCY;
				}
			}
		}
...
{
	// jay - bhop cvar
	if (!strcmp(pmove->PM_Info_ValueForKey(pmove->physinfo, "abh"), "1"))
		return;
...
=============
*/
void PM_DropPunchAngle ( vec3_t punchangle, int amount )
{
	float	len;
	
	len = VectorNormalize ( punchangle );
	//len -= (10.0 + len * 0.5) * pmove->frametime;
	len -= (amount + len * 0.5) * pmove->frametime;
	len = max( len, 0.0 );
...
void PM_CheckParamters( void )
{
	...
	if (pmove->flNextPunchTime <= 0)
	{
		PM_DropPunchAngle(pmove->punchanglejit, 70);
		pmove->flNextPunchTime = 90;
	}

	PM_DropPunchAngle(pmove->punchangle, 10);
	...
	if ( !pmove->dead )
	{
		...
		VectorAdd(v_angle, pmove->punchanglejit, v_angle);

		VectorCopy(pmove->punchanglejit, pmove->vuser4);
...
void PM_ReduceTimers( void )
{
	if (pmove->flTimeLadderPunch > 0)
	{
		pmove->flTimeLadderPunch -= pmove->cmd.msec;
		if (pmove->flTimeLadderPunch < 0)
		{
			pmove->flTimeLadderPunch = 0;
		}
	}
	if (pmove->flNextPunchTime > 0)
	{
		pmove->flNextPunchTime -= pmove->cmd.msec;
		if (pmove->flNextPunchTime < 0)
		{
			pmove->flNextPunchTime = 0;
		}
	}
	if ( pmove->flTimeStepSound > 0 )
	{
