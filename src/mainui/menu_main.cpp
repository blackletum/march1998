...
#define ART_CONSOLE		"gfx/shell/console_btn"
#define	ART_CONSOLE_D	"gfx/shell/console_btn_d"
...
static void UI_Main_Callback( void *self, int event )
{
	...
	case ID_CONSOLE:
		if (event == QM_PRESSED)
			((menuBitmap_s*)self)->focusPic = ART_CONSOLE_D;
		else ((menuBitmap_s*)self)->focusPic = ART_CONSOLE_D;
...
static void UI_Main_Init( void )
{
	...
	uiMain.console.generic.flags = QMF_HIGHLIGHTIFFOCUS | QMF_MOUSEONLY | QMF_ACT_ONRELEASE;
	uiMain.console.generic.x = 20;
	uiMain.console.generic.y = 17;
	uiMain.console.generic.width = 80;
	uiMain.console.generic.height = 30;
	uiMain.console.generic.callback = UI_Main_Callback;
	uiMain.console.pic = ART_CONSOLE;
	uiMain.console.focusPic = ART_CONSOLE_D;
	...
	uiMain.resumeGame.generic.y = 330; //230;
	...
	if (CL_IsActive())
		uiMain.newGame.generic.y = 380;
	else
		uiMain.newGame.generic.y = 330; //280;
	...
	if (CL_IsActive())
		uiMain.hazardCourse.generic.y = 430;
	else
		uiMain.hazardCourse.generic.y = 380; //330;
	...
	if (CL_IsActive())
		uiMain.saveRestore.generic.y = bTrainMap ? 480 : 430;
	else
		uiMain.saveRestore.generic.y = bTrainMap ? 430 : 380; //bTrainMap ? 380 : 330;
	...
	if (CL_IsActive())
		uiMain.configuration.generic.y = bTrainMap ? 530 : 480;
	else
		uiMain.configuration.generic.y = bTrainMap ? 480 : 430; //bTrainMap ? 430 : 380;
	...
	if (CL_IsActive())
		uiMain.multiPlayer.generic.y = bTrainMap ? 580 : 530;
	else
		uiMain.multiPlayer.generic.y = bTrainMap ? 530 : 480;
	...
	if (CL_IsActive())
		...
		uiMain.quit.generic.y = (bTrainMap ? 580 : 530);
