...
void CL_DrawParticles( double frametime )
{
	...
	for( p = cl_active_particles; p; p = p->next )
	{
		if(( p->type != pt_blob ) || ( p->packedColor == 255 ))
		{
			...
			pglColor4ub(LightToTexGamma(pColor->r) * 0.7, LightToTexGamma(pColor->g) * 0.7, LightToTexGamma(pColor->b) * 0.7, alpha);
...
void R_Blood( const vec3_t org, const vec3_t ndir, int pcolor, int speed )
{
	...
	if (pcolor == 19)
	{
		for (j = 0; j < 2; j++)
		{
			dir[0] = ndir[0] + COM_RandomFloat(-0.2f, 0.2f);
			dir[1] = ndir[1] + COM_RandomFloat(-0.2f, 0.2f);

			for (arc = 0.05f, i = 0; i < 100; i++)
			{
				dir[0] = dir[0] + COM_RandomFloat(-0.01f, 0.01f);
				dir[1] = dir[1] + COM_RandomFloat(-0.01f, 0.01f);
				...
				VectorScale(p->vel, (i + 50) * (speed * 0.005), p->vel);
			}
		}

		for (k = 0; k < 45; k++)
		{
			VectorAddScalar(org, COM_RandomFloat(-3.0f, 3.0f), ppos);
			ddir[0] = ndir[0] + COM_RandomFloat(-0.01f, 0.01f);
			ddir[1] = ndir[1] + COM_RandomFloat(-0.01f, 0.01f);
			...
			p->type = pt_vox_slowgrav;
			p->color = pcolor + COM_RandomLong(0, 9);
			VectorCopy(ppos, p->org);
			VectorCopy(ddir, p->vel);

			VectorScale(p->vel, speed * COM_RandomFloat(0.25f, 1.0f), p->vel);
		}
	}
	else
	{
...
===============
*/
// jay - vasia's accurate bloodstreams
void R_BloodStream(const vec3_t org, const vec3_t dir, int pcolor, float speed)
{
	// Add our particles.
	vec3_t		dirCopy;
	float		arc = 0.05;
...
void R_RocketTrail( vec3_t start, vec3_t end, int type )
{
	...
	while( len > 0 )
	{
		...
		case 4:	// slight blood
			...
			len -= 3.0f;
			break;
		case 6:	// headcrab - vasia's reverse engineer of the funny hornet trail
			p->ramp = COM_RandomLong(0, 3);
			p->color = ramp3[(int)p->ramp];
			p->type = pt_fire;
			VectorCopy(start, p->org);
...
{
	R_RunParticleEffect(pos, vec3_origin, 0, 20); //magic nipples - normal impact particles become beta.
...
===============
*/
void R_AlphaSparkShower(const vec3_t pos)
{
	int		i, quantity;
	particle_t* p;

	quantity = COM_RandomLong(10, 12);
	...
	for (i = 0; i < quantity; i++)
	{
		...
		p->vel[0] = COM_RandomFloat(-0.2f, 0.2f);
		p->vel[1] = COM_RandomFloat(-0.2f, 0.2f);
		p->vel[2] = COM_RandomFloat(0.75f, 1.0f);
		VectorScale(p->vel, COM_RandomFloat(25.0f, 75.0f), p->vel);
