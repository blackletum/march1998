...
const char* cl_aexplode_sounds[] =
{
	"weapons/aexplode3.wav",
	"weapons/aexplode4.wav",
	"weapons/aexplode5.wav",
};
...
void CL_LoadClientSprites( void )
{
	...
	cl_sprite_spark = CL_LoadClientSprite(cl_default_sprites[7]);//magic nipples - 98 sparks
}
...
void CL_AddClientResources( void )
{
	...
	for (i = 0; i < ARRAYSIZE(cl_aexplode_sounds); i++)
	{
		Q_snprintf(filepath, sizeof(filepath), "%s%s", DEFAULT_SOUNDPATH, cl_aexplode_sounds[i]);

		if (!FS_FileExists(filepath, false))
			CL_AddClientResource(cl_aexplode_sounds[i], t_sound);
	}
...
void R_RicochetSprite( const vec3_t pos, model_t *pmodel, float duration, float scale )
{
	...
	//pTemp->fadeSpeed = 8;
	pTemp->die = cl.time + 0.03; //magic nipples - changed ricochet to die, and not fade to be like goldsrc.
...
void R_SparkShower( const vec3_t pos )
{
	...
	int		i;
	int nCount = COM_RandomLong(2, 5); //magic nipples - was 5, but e3 footages shows random numbers at spawn.
	float flSize = 0.09;
	float flAmplitude = 90;
	float flSpeed = 90;

	//zmodel = Mod_Handle( CL_FindModelIndex( "sprites/richo1.spr" ));
	R_RicochetSprite(pos, cl_sprite_ricochet, 0.0f, COM_RandomFloat(0.4f, 0.6f));

	VectorSubtract(pos, pos, vecDelta);
	...
	for (i = 0; i < nCount; i++)
	{
		...
		if (i == 0) VectorCopy(pos, vecPos);
		else VectorMA(pos, (i / (nCount - 1.0f)), vecDelta, vecPos);

		pTemp = CL_TempEntAlloc(vecPos, cl_sprite_spark);
		if (!pTemp) return;

		pTemp->flags = (FTENT_COLLIDEWORLD | FTENT_SPRCYCLE | FTENT_GRAVITY);
		...
		vecVel[2] += COM_RandomFloat(128.0f, 256.0f) * flAmplitude; //magic nipples - z axis will be slightly different based on e3 footage
...
void R_Explosion( vec3_t pos, int model, float scale, float framerate, int flags )
{
	...
	if( !FBitSet( flags, TE_EXPLFLAG_NOSOUND ))
	{
		...
		hSound = S_RegisterSound(va("%s", cl_aexplode_sounds[COM_RandomLong(0, 2)]));
...
void CL_AddModelEffects( cl_entity_t *ent )
{
	...
	// headcrab - were not gonna be using this trail lmao, so it's the hornet trail now 
	...
	//magic nipples - alpha explosion needs shrapnel to have orange glow.
	// using wizard trail since that goes unused...
	if (FBitSet(ent->model->flags, STUDIO_TRACER3))
	{
		if (COM_RandomLong(0, 4) == 4)
		{
			dlight_t* dl = CL_AllocDlight(ent->index);
