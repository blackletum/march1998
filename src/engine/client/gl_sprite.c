...
void R_SpriteInit( void )
{
	...
	r_sprite_lighting = Cvar_Get( "r_sprite_lighting", "0", FCVAR_ARCHIVE, "enables sprite lighting (blood etc)" );
}
...
static dframetype_t *R_SpriteLoadFrame( model_t *mod, void *pin, mspriteframe_t **ppframe, int num )
{
	...
	if( FBitSet( mod->flags, MODEL_CLIENT )) // it's a HUD sprite Magic Nipples
	{
		...
		gl_texturenum = GL_LoadTexture( texname, pin, pinframe->width * pinframe->height * bytes, TF_IMAGE);
	}
	else
	{
