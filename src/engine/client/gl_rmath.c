...
void V_AdjustFov( float *fov_x, float *fov_y, float width, float height, qboolean lock_x )
{
	...
	{
		// 4:3 or 5:4 ratio
		// magic nipples - the alpha stretches the y axis in the game slightly so this replicates that
		*fov_y = 2 * atan((width * 3) / (height * 4.6) * tan(*fov_y * M_PI / 360.0 * 0.5)) * 360 / M_PI;
