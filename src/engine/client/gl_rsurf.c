...
void R_DrawWaterSurfaces( void )
{
	...
	for( i = 0; i < cl.worldmodel->numtextures; i++ )
	{
		...
		R_UploadRipples(t); //GL_Bind( GL_TEXTURE0, t->gl_texturenum );
...
void R_DrawBrushModel( cl_entity_t *e )
{
	...
	//magic nipples - darken translucent brushes
	if (e->curstate.rendermode == kRenderTransColor)
	{
		if ((e->curstate.renderamt < 100) && ((e->curstate.renderamt > 0)))
		{
			pglColor4ub(
				e->curstate.rendercolor.r * 0.11,
				e->curstate.rendercolor.g * 0.11,
				e->curstate.rendercolor.b * 0.11,
				e->curstate.renderamt * 5
			);
		}
		else
		{
			pglColor4ub(
				e->curstate.rendercolor.r * 0.9,
				e->curstate.rendercolor.g * 0.9,
				e->curstate.rendercolor.b * 0.9,
	...
	if (e->curstate.rendermode == kRenderTransTexture)
	{
		...
		pglColor4f(0.5f, 0.5f, 0.5f, tr.blend);
	}
