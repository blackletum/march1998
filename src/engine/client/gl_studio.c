...
void R_StudioDynamicLight( cl_entity_t *ent, alight_t *plight )
{
	...
	plight->shadelight = plight->shadelight * 0.75;
	plight->ambientlight = plight->ambientlight * 0.75;
	...
	while( i = *( ptricmds++ ))
	{
		...
		for( ; i > 0; i--, ptricmds += 4 )
		{
			...
			if (r_warp->value > 0)
			{
				if (RI.currententity == &clgame.viewent)
				{
					g_studio.verts[ptricmds[0]][0] = floor(35 * g_studio.verts[ptricmds[0]][0]) / 35;
					g_studio.verts[ptricmds[0]][1] = floor(35 * g_studio.verts[ptricmds[0]][1]) / 35;
					g_studio.verts[ptricmds[0]][2] = floor(35 * g_studio.verts[ptricmds[0]][2]) / 35;
				}
				else
				{
					g_studio.verts[ptricmds[0]][0] = floor(6 * g_studio.verts[ptricmds[0]][0]) / 6;
					g_studio.verts[ptricmds[0]][1] = floor(6 * g_studio.verts[ptricmds[0]][1]) / 6;
					g_studio.verts[ptricmds[0]][2] = floor(6 * g_studio.verts[ptricmds[0]][2]) / 6;
				}
			}
