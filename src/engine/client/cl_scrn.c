...
qboolean SCR_LoadVariableWidthFont( const char *fontname )
{
	...
	cls.creditsFont.hFontTexture = GL_LoadTexture( fontname, NULL, 0, TF_IMAGE| TF_NEAREST); // headcrab - quake font nearest shit lol
	R_GetTextureParms( &fontWidth, NULL, cls.creditsFont.hFontTexture );
...
{
	if( !SCR_LoadVariableWidthFont( "gfx/conchars" ))
	{
...
void SCR_Init( void )
{
	...
	cl_broken_hud = Cvar_Get("broken_hud", "0", FCVAR_ARCHIVE, "add black backing to hud like alpha photos"); //magic nipples 
