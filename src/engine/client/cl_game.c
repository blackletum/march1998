...
======== =
*/
static void pfnSPR_DrawAdditiveHud(int frame, int x, int y, const wrect_t * prc)
{
	pglEnable(GL_BLEND);
...
static void pfnGetPlayerInfo( int ent_num, hud_player_info_t *pinfo )
{
	...
	pinfo->scimodel = player->scimodel;
...
static int pfnDrawCharacter( int x, int y, int number, int r, int g, int b )
{
	...
	pfnPIC_DrawHoles( x, y, 20, 35, &cls.creditsFont.fontRc[number] ); // headcrab - quake credid font scale change
...
void CL_UnloadProgs( void )
{
	...
	if( Q__stricmp( GI->gamedir, "hlfx" ) || GI->version != 0.5f )
