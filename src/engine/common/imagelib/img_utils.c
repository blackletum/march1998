...
void Image_SetPaletteMag(const byte* pal, uint* d_table)
{
	byte	rgba[4];
	int	i, j;// , k;
...
void Image_GetPaletteLMPMag(const byte* pal, int rendermode)
{
	image.d_rendermode = rendermode;

	if (pal)
	{
		Image_SetPaletteMag(pal, d_8to24table);
		image.d_currentpal = d_8to24table;
	}
	else
	{
...
byte *Image_CreateLumaInternal( byte *fin, int width, int height, int type, int flags )
{
	...
	case PF_INDEXED_32:
		...
		for( i = 0; i < width * height; i++ )
			*out++ = fin[i] >= 224 ? fin[i] : 223;
		// headcrab - most glow textures specifically make this color black, I think this is intentionally a index used for them
		break;
	default:
