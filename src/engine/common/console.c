...
void Con_DrawCharCrosshair(int x, int y, int number, int scale)
{
	int		width, height;
	float		s1, t1, s2, t2;
	gl_texture_t* glt;
	...
	pglColor4ub(255, 255, 255, 255); //magic nipples - just solid white please
	R_GetTextureParms(&width, &height, con.curFont->hFontTexture);
	...
	con.curFont->charHeight = height * scale;
	...
	R_DrawStretchPic(x, y - 1, width, height + 6, s1, t1, s2, t2, con.curFont->hFontTexture);
	pglColor4ub(255, 255, 255, 255); // don't forget reset color
...
void Con_DrawSolidConsole( int lines )
{
	...
	if( host.allow_console )
	{
		...
		Q_snprintf(curbuild, MAX_STRING, "Half-Life alpha v 0.61", XASH_VERSION, Q_buildnum());
