...
void UTIL_HudMessage( CBaseEntity *pEntity, const hudtextparms_t &textparms, const char *pMessage )
{
	...
	EMIT_SOUND(ENT(pEntity->edict()), CHAN_STATIC, "Common/TALK.WAV", 1.0, ATTN_NORM);
...
void UTIL_Blood(const Vector& origin, const Vector& direction, int color, int amount)
{
...
float UTIL_SmoothValue(float startValue, float endValue, float speed)
{
...
float UTIL_SmoothAngle(float startValue, float endValue, float finalValue, float speed)
{
	float absd, d;
