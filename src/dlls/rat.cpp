...
void CRat::Spawn()
{
	...
	UTIL_SetSize(pev, Vector(-6, -6, 0), Vector(6, 6, 4));
...
void CRat::Killed(entvars_t* pevAttacker, int iGib)
{
	...
	if (RANDOM_LONG(0, 4) == 1)
		EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "rat/rat_die1.wav", 0.8, ATTN_NORM, 0, 80 + RANDOM_LONG(0, 39));
	else
		EMIT_SOUND_DYN(ENT(pev), CHAN_BODY, "rat/rat_die2.wav", 0.7, ATTN_NORM, 0, 80 + RANDOM_LONG(0, 39));
...
void CRat::MonsterThink(void)
{
	...
	if (pev->health > 0)
	{
		...
		case RAT_SCARED_BY_LIGHT:
		{
			...
			{
				m_flLastLightLevel = GETENTITYILLUM(ENT(pev));
				SetActivity(ACT_RUN); //Magic Nipples - Might of fixed the rat getting stuck in idle.
			}
			break;
		}
		}
