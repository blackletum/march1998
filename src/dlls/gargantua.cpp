...
public:
	...
	void SetObjectCollisionBox(void)
	{
		...
		pev->absmax = pev->origin + Vector(72, 72, 192);
	}
...
const char* CGargantua::pDeathSounds[] =
{
	"garg/gar_die1.wav",
	"garg/gar_die2.wav",
};
...
void CGargantua::Spawn()
{
	...
	if (FBitSet(pev->spawnflags, SF_MONSTER_WAIT_FOR_SCRIPT))
		m_MonsterState = MONSTERSTATE_SCRIPT;	// very much needed on c2a1!!!
	else
...
{
	EMIT_SOUND_DYN(edict(), CHAN_VOICE, RANDOM_SOUND_ARRAY(pDeathSounds), 1.0, ATTN_IDLE, 0, 100);
}
