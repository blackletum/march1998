...
TYPEDESCRIPTION	CBasePlayer::m_playerSaveData[] = 
{
	...
	DEFINE_FIELD(CBasePlayer, m_flNextConcussTime, FIELD_TIME),
...
void LinkUserMessages( void )
{
	...
	gmsgPunchAngle = REG_USER_MSG("PunchAngle", -1);
...
void CBasePlayer :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	//if ( pev->takedamage )
	{
		...
		if ((pev->health - flDamage) <= 0)
			UTIL_BloodStream(ptr->vecEndPos, UTIL_RandomBloodVector(), BloodColor(), RANDOM_LONG(100, 175));
		else
			UTIL_Blood(ptr->vecEndPos, UTIL_RandomBloodVector(), BloodColor(), RANDOM_LONG(1, 10));
	}
}
...
int CBasePlayer :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	m_iDamagePool += flDamage * 2.25;
	if (m_iDamagePool >= 180)
		m_iDamagePool = 180;

	float dmgfadeamt = 200 - m_iDamagePool * 2.5;
	if (dmgfadeamt <= 15)
		dmgfadeamt = 15;

	m_iDamagePoolTimer = gpGlobals->time + 1.1;

	//UTIL_ScreenFade(this, Vector(255, 0, 0), 1, 0.2, m_iDamagePool, 0);
	...
	while (fTookDamage && (!ftrivial || (bitsDamage & DMG_TIMEBASED)) && ffound && bitsDamage)
	{
		...
		if (bitsDamage & (DMG_POISON | DMG_PARALYZE))
		{
			...
			ffound = TRUE;
			UTIL_ScreenFade((CBaseEntity*)this, Vector(0, 255, 0), 1, 0.1, 100, FFADE_MODULATE);
		}
		...
		if (bitsDamage & DMG_RADIATION)
		{
			if ((pev->radsuit_finished < gpGlobals->time) && (m_rgItems[ITEM_RADIATION]))
			{
				m_bitsDamageType &= ~(0);
				m_rgbTimeBasedDamage[0] = 0;

				pev->radsuit_finished = gpGlobals->time + 30;

				m_rgItems[ITEM_RADIATION]--;

				EMIT_SOUND_SUIT(edict(), "!HEV_HEAL4"); // radiation shot administered

				UTIL_ScreenFade((CBaseEntity*)this, Vector(255, 255, 255), 1, 0.5, 125, FFADE_MODULATE);
			}
			else if (pev->radsuit_finished < gpGlobals->time)
			{
				SetSuitUpdate("!HEV_DET2", FALSE, SUIT_NEXT_IN_1MIN);	// radiation detected
				bitsDamage &= ~DMG_RADIATION;
				ffound = TRUE;
				UTIL_ScreenFade((CBaseEntity*)this, Vector(0, 255, 0), 1, 0.1, 100, 2);
			}
		}
	...
	else
	{
		...
		flDot = DotProduct(gpGlobals->v_forward, g_vecAttackDir);
		pev->punchangle.x = flDot * 2;

		flDot = DotProduct(gpGlobals->v_right, g_vecAttackDir);
		pev->punchangle.z = flDot * 2;

		flDot = DotProduct(gpGlobals->v_up, g_vecAttackDir * -1);
		pev->punchangle.x += fabs(flDot) * 2;
...
void CBasePlayer::Killed(entvars_t* pevAttacker, int iGib)
{
	...
	if (fGibbed)
	{
		...
		m_fCanRevive = FALSE;
		UTIL_ScreenFade(this, Vector(128, 0, 0), 8.8, 15, 255, FFADE_OUT | FFADE_MODULATE);
	}
...
#define AIRTANKTIME	60		// magic nipples - airtank has this many seconds
...
void CBasePlayer::WaterMove()
{
	...
	if (pev->waterlevel == 3)
	{
		if (m_rgItems[ITEM_OXYGEN])
		{
			UTIL_ScreenFade((CBaseEntity*)this, Vector(150, 200, 255), 1, 0.1, 50, FFADE_MODULATE); //screen overlay to show difference when using air tank

			if (oxyBreathTime < gpGlobals->time)
			{
				EMIT_SOUND(ENT(pev), CHAN_VOICE, "player/wetsuit.wav", 0.9, ATTN_NORM);

				oxyBreathTime = gpGlobals->time + RANDOM_LONG(5, 6);

				m_fOxygen--;

				pev->air_finished = gpGlobals->time + AIRTIME; // put this here so you still have normal breath when tank runs out
			}
			else if (m_fOxygen < 1)
			{
				m_fOxygen = 0;

				m_rgItems[ITEM_OXYGEN]--;

				MESSAGE_BEGIN(MSG_ONE, gmsgOxygen, NULL, pev);
				WRITE_BYTE(m_rgItems[ITEM_OXYGEN]);
				MESSAGE_END();

				pev->air_finished = gpGlobals->time + AIRTIME; // ditto to previous comment
			}
			//try to delay bubbles  until you breath out
			if ((oxyBreathTime < gpGlobals->time + 4.5) && (oxyBubbleTime < gpGlobals->time))
			{
				UTIL_Bubbles(pev->origin - Vector(24, 24, 0), pev->origin + Vector(24, 24, 48), 100);
				oxyBubbleTime = gpGlobals->time + 5;
			}
		}
	}
	...
	{
		// not underwater
		STOP_SOUND(ENT(pev), CHAN_VOICE, "player/wetsuit.wav");
		...
		oxyBreathTime = gpGlobals->time + 2;
	...
	if (m_fOxygen < 1)
	{
		if (!RANDOM_LONG(0, 0x1f) && RANDOM_LONG(0, AIRTIME - 1) >= air)
		{
			...
			UTIL_Bubbles(pev->origin - Vector(8, 8, 8), pev->origin + Vector(8, 8, 8), 16);
		}
	}
	...
	for (i = 0; i < CDMG_TIMEBASED; i++)
	{
		...
		if (m_bitsDamageType & (DMG_PARALYZE << i))
		{
			...
			case itbd_Poison:
				...
				{
					//EMIT_SOUND_SUIT(edict(), "!HEV_HEAL5"); // antidote shot administered
				}
				break;
			case itbd_Radiation:
				if (m_rgItems[ITEM_RADIATION])
				{
					m_rgbTimeBasedDamage[itbd_Radiation] = 0;
					m_bitsDamageType &= ~(DMG_PARALYZE << i);
				}
				else if (pev->radsuit_finished >= gpGlobals->time) //magic nipples - hack used to fix taking radiation dmg when you had 0 rad suits but still had time left
				{
					m_rgbTimeBasedDamage[itbd_Radiation] = 0;
					m_bitsDamageType &= ~(DMG_PARALYZE << i);
				}
			...
			if (m_rgbTimeBasedDamage[i])
			{
					...
					(i == itbd_Paralyze))
				{
					if (m_rgItems[ITEM_ANTIDOTE])
					{
						...
						MESSAGE_BEGIN(MSG_ONE, gmsgAntidote, NULL, pev);
						WRITE_BYTE(m_rgItems[ITEM_ANTIDOTE]);
						MESSAGE_END();
					}
				}
				...
				if (i == itbd_Radiation)
				{
					if (m_rgItems[ITEM_RADIATION])
					{
						...
						WRITE_BYTE(m_rgItems[ITEM_RADIATION]);
						MESSAGE_END();
					}
				}
...
Augmentation 
	...
	Long Jump
		Used by hitting the ??? key(s). Causes the player to jump further than normal.
	SCUBA	
...
// jay - bhop cvar
void CBasePlayer::CheckBHop()
{
	if (CVAR_GET_FLOAT("sv_allowbhop") > 0)
		g_engfuncs.pfnSetPhysicsKeyValue(edict(), "abh", "1");
	else
		g_engfuncs.pfnSetPhysicsKeyValue(edict(), "abh", "0");
}

void CBasePlayer::PostThink()
{

		g_engfuncs.pfnSetPhysicsKeyValue(edict(), "indp", "1");

	CheckBHop();	// jay - bhop cvar

	if (m_iDamagePoolTimer < gpGlobals->time)
	{
		if (m_iDamagePool > 0)
		{
			//m_iDamagePool--;
			//m_iDamagePoolTimer = gpGlobals->time + 0.01;
			m_iDamagePool = 0;
		}
	}
	//ALERT(at_console, "%f\n", m_iDamagePool);

	MESSAGE_BEGIN(MSG_ALL, gmsgPunchAngle, NULL);
		WRITE_COORD(pev->vuser4[0]);
		WRITE_COORD(pev->vuser4[1]);
		WRITE_COORD(pev->vuser4[2]);
	MESSAGE_END();

	if (m_flNextConcussTime < gpGlobals->time)
	{
...
// fallpain sound, and damage will be inflicted based on how far the player fell
	...
	// if we have oxygen tank, then he will be on player model
	if (m_rgItems[ITEM_OXYGEN])
		SetBodygroup(1, 1);
	else
		SetBodygroup(1, 0);
	...
	// ????		Icons
		...
		for (i = 0; i < MAX_WEAPONS; i++)
		{
			...
			MESSAGE_BEGIN( MSG_ONE, gmsgWeaponList, NULL, pev );  
				...
				WRITE_BYTE(II.iMaxClip);				// byte		Flags
			MESSAGE_END();
		}
	}
