...
#define SF_MONSTERMAKER_ISPORTAL	16 // Alpha portal FX
...
TYPEDESCRIPTION	CMonsterMaker::m_SaveData[] =
{
	...
	DEFINE_FIELD(CMonsterMaker, m_iszMonsterNetname, FIELD_STRING),
};
...
void CMonsterMaker::KeyValue(KeyValueData* pkvd)
{
	...
	else if (FStrEq(pkvd->szKeyName, "m_iszMonsterNetname"))
	{
		m_iszMonsterNetname = ALLOC_STRING(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else
...
void CMonsterMaker::Spawn()
{
	...
	if (!FStringNull(pev->targetname))
	{
		...
		{
			//	ALERT ( at_console, "CMonsterMaker: Cyclic use was set!!\n" );
	...
	{// no targetname, just start.
		ALERT(at_console, "CMonsterMaker: targetname was null!!\n");
...
void CMonsterMaker::Precache(void)
{
	...
	if (FBitSet(pev->spawnflags, SF_MONSTERMAKER_ISPORTAL))
	{
...
void CMonsterMaker::MakeMonster(void)
{
	...
	//	ALERT ( at_console, "CMonsterMaker: MakeMonster!!\n" );
	...
	// don't do "stack" check for portal monstermaker; teledeath will remove anyone who is in the way
	if (FBitSet(pev->spawnflags, SF_MONSTERMAKER_ISPORTAL))
	{
		// make portal effect
	...
	if (FBitSet(pev->spawnflags, SF_MONSTERMAKER_ISPORTAL))
	{
		...
		//		pEntity->pev->effects |= EF_NOINTERP;

		pEntity->pev->rendermode = kRenderTransTexture;
		pEntity->pev->renderamt = 0;

		pEntity->SUB_StartFadeIn();

		if (pEntity->Classify())
		{
			CBaseMonster* pMonster;

			pMonster = (CBaseMonster*)Instance(pEntity->edict());
	...
	if (!FStringNull(m_iszMonsterNetname))
	{
		pevCreate->netname = m_iszMonsterNetname;
	}
