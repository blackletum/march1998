...
enum
{
	SCHED_ROBO_SUPPRESS = LAST_COMMON_SCHEDULE + 1,
	SCHED_ROBO_THREAT_DISPLAY,
};
...
enum
{
	TASK_ROBO_SETUP_HIDE_ATTACK = LAST_COMMON_TASK + 1,
	TASK_ROBO_GET_PATH_TO_ENEMY_CORPSE,
};

int iroboMuzzleFlash;

//=========================================================
// Monster's Anim Events Go Here
//=========================================================
#define		ROBO_AE_HORNET1	( 1 )
#define		ROBO_AE_HORNET2	( 2 )
#define		ROBO_AE_HORNET3	( 3 )
#define		ROBO_AE_HORNET4	( 4 )
#define		ROBO_AE_HORNET5	( 5 )
// some events are set up in the QC file that aren't recognized by the code yet.
#define		ROBO_AE_PUNCH		( 6 )
#define		ROBO_AE_BITE		( 7 )

#define		ROBO_AE_LEFT_FOOT	 ( 10 )
#define		ROBO_AE_RIGHT_FOOT ( 11 )

#define		ROBO_AE_LEFT_PUNCH ( 12 )
#define		ROBO_AE_RIGHT_PUNCH ( 13 )


//=========================================================
// Unused Grunt beam
//=========================================================
class CRoboLazer : public CBaseAnimating
{
public:
	void Spawn(void);
...
LINK_ENTITY_TO_CLASS(robolazer, CRoboLazer);

TYPEDESCRIPTION	CRoboLazer::m_SaveData[] =
{
	DEFINE_FIELD(CRoboLazer, m_maxFrame, FIELD_INTEGER),
};

IMPLEMENT_SAVERESTORE(CRoboLazer, CBaseEntity);

void CRoboLazer::Spawn(void)
{
...
void CRoboLazer::Animate(void)
{
	...
	pev->angles.x = 0; // headcrab - real alpha lol

	EMIT_SOUND(ENT(pev), CHAN_WEAPON, "agrunt/lasfly.wav", 1, ATTN_NORM);
...
void CRoboLazer::Shoot(entvars_t* pevOwner, Vector vecStart, Vector vecVelocity)
{
	CRoboLazer* pSpit = GetClassPtr((CRoboLazer*)NULL);
	pSpit->Spawn();
	...
	pSpit->SetThink(&CRoboLazer::Animate);
	pSpit->pev->nextthink = gpGlobals->time + 0.1;
}

void CRoboLazer::Touch(CBaseEntity* pOther)
{
	TraceResult tr;
	int		iPitch;
	SetTouch(NULL);
	...
	{
	case 0:
		EMIT_SOUND(ENT(pev), CHAN_WEAPON, "agrunt/lashit.wav", 1, ATTN_NORM); break;
	case 1:
		EMIT_SOUND(ENT(pev), CHAN_WEAPON, "agrunt/lashit.wav", 1, ATTN_NORM); break;
	}
	...
	SetThink(&CRoboLazer::SUB_Remove);
	pev->nextthink = gpGlobals->time;
}


#define		ROBO_MELEE_DIST	100

class CRobo : public CSquadMonster
{
public:
	void Spawn(void);
	void Precache(void);
...
LINK_ENTITY_TO_CLASS(monster_robot, CRobo);

TYPEDESCRIPTION	CRobo::m_SaveData[] =
{
	DEFINE_FIELD(CRobo, m_fCanHornetAttack, FIELD_BOOLEAN),
	DEFINE_FIELD(CRobo, m_flNextHornetAttackCheck, FIELD_TIME),
	DEFINE_FIELD(CRobo, m_flNextPainTime, FIELD_TIME),
	DEFINE_FIELD(CRobo, m_flNextSpeakTime, FIELD_TIME),
	DEFINE_FIELD(CRobo, m_flNextWordTime, FIELD_TIME),
	DEFINE_FIELD(CRobo, m_iLastWord, FIELD_INTEGER),
};

IMPLEMENT_SAVERESTORE(CRobo, CSquadMonster);

const char* CRobo::pAttackHitSounds[] =
{
...
const char* CRobo::pAttackMissSounds[] =
{
	"zombie/claw_miss1.wav",
	"zombie/claw_miss2.wav",
};

const char* CRobo::pAttackSounds[] =
{
...
//=========================================================
int CRobo::IRelationship(CBaseEntity* pTarget)
{
...
// TraceAttack
//=========================================================
void CRobo::TraceAttack(entvars_t* pevAttacker, float flDamage, Vector vecDir, TraceResult* ptr, int bitsDamageType)
{
...
// PrescheduleThink 
//=========================================================
void CRobo::PrescheduleThink(void)
{
	if (ShouldSpeak())
	{
...
//=========================================================
void CRobo::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	switch (pEvent->event)
	{
	case ROBO_AE_HORNET1:
	case ROBO_AE_HORNET2:
	case ROBO_AE_HORNET3:
	case ROBO_AE_HORNET4:
	case ROBO_AE_HORNET5:
	{
		...
		Vector angDir;
			...
			//GetAttachment(2, vecArm2Pos, vecArm2Dir);

			// !!!HACKHACK - the spot at which the spit originates (in front of the mouth) was measured in 3ds and hardcoded here.
			// we should be able to read the position of bones at runtime for this info.
			vecSpitOffset = (gpGlobals->v_right * 8 + gpGlobals->v_forward * 37 + gpGlobals->v_up * 23);
			vecSpitOffset = (pev->origin + vecSpitOffset);
			vecSpitDir = ((m_hEnemy->pev->origin + m_hEnemy->pev->view_ofs) - vecArmPos).Normalize();
			//vecSpit2Dir = ((m_hEnemy->pev->origin + m_hEnemy->pev->view_ofs) - vecArm2Pos).Normalize();

			vecSpitDir.x += RANDOM_FLOAT(-0.05, 0.05);
			vecSpitDir.y += RANDOM_FLOAT(-0.05, 0.05);
			vecSpitDir.z += RANDOM_FLOAT(-0.07, -0.02);

			EMIT_SOUND(ENT(pev), CHAN_WEAPON, "agrunt/laser2.wav", 1, ATTN_NORM);
			CRoboLazer::Shoot(pev, vecArmPos, vecSpitDir * 900);
			//CRoboLazer::Shoot(pev, vecArm2Pos, vecSpit2Dir * 900);

	}
	break;

	case ROBO_AE_LEFT_FOOT:
		switch (RANDOM_LONG(0, 1))
		{
			// left foot
	...
	case ROBO_AE_LEFT_PUNCH:
	{
		CBaseEntity* pHurt = CheckTraceHullAttack(ROBO_MELEE_DIST, gSkillData.agruntDmgPunch, DMG_CLUB);
...
void CRobo::Precache()
{
	...
	iroboMuzzleFlash = PRECACHE_MODEL("sprites/muz4.spr");
...
Schedule_t	slroboFail[] =
{
	{
		tlroboFail,
		ARRAYSIZE(tlroboFail),
...
Schedule_t	slroboCombatFail[] =
{
	{
		tlroboCombatFail,
		ARRAYSIZE(tlroboCombatFail),
		bits_COND_CAN_RANGE_ATTACK1 |
		bits_COND_CAN_MELEE_ATTACK1,
		0,
		"robo Combat Fail"
...
Schedule_t slroboStandoff[] =
{
	{
		tlroboStandoff,
		ARRAYSIZE(tlroboStandoff),
		bits_COND_CAN_RANGE_ATTACK1 |
		bits_COND_CAN_MELEE_ATTACK1 |
		bits_COND_SEE_ENEMY |
		bits_COND_NEW_ENEMY |
...
// Suppress
//=========================================================
Task_t	tlroboSuppressHornet[] =
{
	{ TASK_STOP_MOVING,		(float)0		},
	{ TASK_RANGE_ATTACK1,	(float)0		},
};

Schedule_t slroboSuppress[] =
{
	{
		tlroboSuppressHornet,
		ARRAYSIZE(tlroboSuppressHornet),
		0,
		0,
		"robo Suppress Hornet",
	},
};
...
Schedule_t	slroboRangeAttack1[] =
{
	{
		tlroboRangeAttack1,
		ARRAYSIZE(tlroboRangeAttack1),
		bits_COND_NEW_ENEMY |
...
Task_t	tlroboHiddenRangeAttack1[] =
{
	{ TASK_SET_FAIL_SCHEDULE,			(float)SCHED_STANDOFF },
	{ TASK_ROBO_SETUP_HIDE_ATTACK,	0				},
	{ TASK_STOP_MOVING,					0				},
	{ TASK_FACE_IDEAL,					0				},
	{ TASK_RANGE_ATTACK1_NOTURN,		(float)0		},
};

Schedule_t	slroboHiddenRangeAttack[] =
{
	{
		tlroboHiddenRangeAttack1,
		ARRAYSIZE(tlroboHiddenRangeAttack1),
		bits_COND_NEW_ENEMY |
		bits_COND_HEAVY_DAMAGE |
		bits_COND_HEAR_SOUND,

		bits_SOUND_DANGER,
		"robo Hidden Range Attack1"
	},
};
...
// cover! 
//=========================================================
Task_t	tlroboTakeCoverFromEnemy[] =
{
	{ TASK_STOP_MOVING,				(float)0					},
	{ TASK_WAIT,					(float)0.2					},
...
Schedule_t	slroboTakeCoverFromEnemy[] =
{
	{
		tlroboTakeCoverFromEnemy,
		ARRAYSIZE(tlroboTakeCoverFromEnemy),
		bits_COND_NEW_ENEMY,
		0,
		"roboTakeCoverFromEnemy"
	},
};
...
// Victory dance!
//=========================================================
Task_t	tlroboVictoryDance[] =
{
	{ TASK_STOP_MOVING,						(float)0					},
	{ TASK_SET_FAIL_SCHEDULE,				(float)SCHED_ROBO_THREAT_DISPLAY	},
	{ TASK_WAIT,							(float)0.2					},
	{ TASK_ROBO_GET_PATH_TO_ENEMY_CORPSE,	(float)0					},
...
Schedule_t	slroboVictoryDance[] =
{
	{
		tlroboVictoryDance,
		ARRAYSIZE(tlroboVictoryDance),
		bits_COND_NEW_ENEMY |
...
Schedule_t	slroboThreatDisplay[] =
{
	{
		tlroboThreatDisplay,
		ARRAYSIZE(tlroboThreatDisplay),
		bits_COND_NEW_ENEMY |
		bits_COND_LIGHT_DAMAGE |
		bits_COND_HEAVY_DAMAGE,
...
DEFINE_CUSTOM_SCHEDULES(CRobo)
{
	slroboFail,
		...
		slroboSuppress,
		slroboRangeAttack1,
		slroboHiddenRangeAttack,
		slroboTakeCoverFromEnemy,
		slroboVictoryDance,
		slroboThreatDisplay,
};

IMPLEMENT_CUSTOM_SCHEDULES(CRobo, CSquadMonster);
...
//=========================================================
BOOL CRobo::FCanCheckAttacks(void)
{
...
//=========================================================
BOOL CRobo::CheckMeleeAttack1(float flDot, float flDist)
{
	if (HasConditions(bits_COND_SEE_ENEMY) && flDist <= ROBO_MELEE_DIST && flDot >= 0.6 && m_hEnemy != NULL)
	{
		return TRUE;
	}
	return FALSE;
...
//=========================================================
BOOL CRobo::CheckRangeAttack1(float flDot, float flDist)
{
	...
	if (HasConditions(bits_COND_SEE_ENEMY) && flDist >= ROBO_MELEE_DIST && flDist <= 1024 && flDot >= 0.5 && NoFriendlyFire())
	{
		TraceResult	tr;
		Vector	vecArmPos, vecArmDir;
...
// StartTask
//=========================================================
void CRobo::StartTask(Task_t* pTask)
{
	switch (pTask->iTask)
	{
	case TASK_ROBO_GET_PATH_TO_ENEMY_CORPSE:
	{
		UTIL_MakeVectors(pev->angles);
		...
		{
			ALERT(at_aiconsole, "roboGetPathToEnemyCorpse failed!!\n");
			TaskFail();
		}
	}
	break;

	case TASK_ROBO_SETUP_HIDE_ATTACK:
		...
		// ideally, this spot is along a line that is perpendicular to a line drawn from the robo to the enemy.
...
// of the proper type.
//=========================================================
Schedule_t* CRobo::GetSchedule(void)
{
...
//=========================================================
//=========================================================
Schedule_t* CRobo::GetScheduleOfType(int Type)
{
	switch (Type)
	{
	case SCHED_TAKE_COVER_FROM_ENEMY:
		return &slroboTakeCoverFromEnemy[0];
		break;
	...
	case SCHED_RANGE_ATTACK1:
		...
		{
			//normal attack
			return &slroboRangeAttack1[0];
		}
		else
		{
			// attack an unseen enemy
			// return &slroboHiddenRangeAttack[ 0 ];
			return &slroboRangeAttack1[0];
		}
		break;

	case SCHED_ROBO_THREAT_DISPLAY:
		return &slroboThreatDisplay[0];
		break;

	case SCHED_ROBO_SUPPRESS:
		return &slroboSuppress[0];
		break;

	case SCHED_STANDOFF:
		return &slroboStandoff[0];
		break;

	case SCHED_VICTORY_DANCE:
		return &slroboVictoryDance[0];
		break;
