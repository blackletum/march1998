...
void CGrenade::Explode( TraceResult *pTrace, int bitsDamageType )
{
	...
	flRndSound = RANDOM_FLOAT( 0 , 1 );
		...
		{
		case 0:	EMIT_SOUND(ENT(pev), CHAN_VOICE, "weapons/debris1.wav", 1, ATTN_NORM);	break;
		case 1:	EMIT_SOUND(ENT(pev), CHAN_VOICE, "weapons/debris2.wav", 1, ATTN_NORM);	break;
		case 2:	EMIT_SOUND(ENT(pev), CHAN_VOICE, "weapons/debris3.wav", 1, ATTN_NORM);	break;
		}
	...
	if (iContents != CONTENTS_WATER)
	{
		...
		for (int i = 0; i < sparkCount; i++)
			Create("dust_shower", pev->origin, pTrace->vecPlaneNormal, NULL);
...
CGrenade* CGrenade::ShootTimedMP5(entvars_t* pevOwner, Vector vecStart, Vector vecVelocity, float time)
{
	CGrenade* pGrenade = GetClassPtr((CGrenade*)NULL);
	pGrenade->Spawn();
	UTIL_SetOrigin(pGrenade->pev, vecStart);
	...
	pGrenade->pev->avelocity.y = 200;
	pGrenade->pev->avelocity.x = 200;

	pGrenade->pev->gravity = 0.5;
	pGrenade->pev->friction = 0.8;

	pGrenade->pev->dmg = 170;
