...
{
#ifndef CLIENT_DLL
	// dont wanna bother with ItemInfo and make a generic function for this so ill just copy this from the snarknest
	if (useType == USE_SET)
	{
		...
		// can I have this?
		if (!g_pGameRules->CanHaveAmmo(pPlayer, "Satchel Charge", SATCHEL_MAX_CARRY))
		{
		...
		if (pOwner != NULL)
		{
			...
			pOwner->DeathNotice(pev);	// spawned by monstermaker
		}

		UTIL_Remove(this);

		if (pActivator->edict() != pev->owner) // dayym! samwa steal ma baam
			return;
		...
		{
			if (FClassnameIs(pSatchel->pev, "monster_satchel") && pSatchel->edict() != this->edict())
			{
				//	ALERT( at_console, "found satchel\n");
				if (pSatchel->pev->owner == pPlayer->edict())
				{
					// there is still an active satchel lying somewhere, don't remove the radio
					return;
				}
			}
		}

		// are we holding radio right now?
		if (FClassnameIs(pPlayer->m_pActiveItem->pev, "weapon_satchel"))
		{
			//	pPlayer->m_pActiveItem->Holster();
			//	pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + 2.1;
