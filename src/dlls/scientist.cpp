...
enum { HEAD_GLASSES = 0, HEAD_EINSTEIN = 1, HEAD_EGON = 2, HEAD_CRAZY = 3 };
...
private:
	...
	float m_healTime;
	float m_fearTime;
	float m_flNextFearDisplay;
	float m_flNextMonsterAnnouncement;
};
...
//=========================================================
// IRelationship - overriden for scientst so we can make 
// him run from the evil Barney.
//=========================================================
...
void CScientist::Spawn(void)
{
	...
	else if (pev->body == HEAD_CRAZY) pev->body = 0;	//	don't use crazy head if predisaster
...
void CScientist::TalkInit()
{
	...
	case HEAD_EGON:		m_voicePitch = 95;  break;	//luther
	case HEAD_CRAZY:	m_voicePitch = 100;  break;//slick
	}
}
...
void CScientist::PainSound(void)
{
	...
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, pPainSounds[RANDOM_LONG(0, ARRAYSIZE(pPainSounds) - 1)], 1, ATTN_NORM, 0, GetVoicePitch());
}
...
{
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, pDeathSounds[RANDOM_LONG(0, ARRAYSIZE(pDeathSounds) - 1)], 1, ATTN_NORM, 0, GetVoicePitch());
}
...
Schedule_t* CScientist::GetSchedule(void)
{
	...
	case MONSTERSTATE_IDLE:
		...
		if (IsFollowing())
		{
			...
			{
				if (HasConditions(bits_COND_NEW_ENEMY) && m_flNextFearDisplay < gpGlobals->time) // I just saw something new and scary, react
				{
					m_flNextFearDisplay = gpGlobals->time + RANDOM_LONG(6, 16);
...
{
	if (flDist <= 64 && flDot >= 0.7 && m_flNextAttack <= gpGlobals->time)
	{
		return TRUE;
	}
	return FALSE;
...
void CScientist::Punch(void)
{
	CBaseEntity* pHurt = CheckTraceHullAttack(70, 5, DMG_SLASH);
	...
	if (pHurt)
	{
		if (pHurt->pev->flags & (FL_MONSTER | FL_CLIENT))
		{
			...
			pHurt->pev->velocity = pHurt->pev->velocity - gpGlobals->v_right * 50;
		}
...
void CDeadScientist::Spawn()
{
	...
	if (pev->sequence == LookupSequence("corpsefloat1") || pev->sequence == LookupSequence("corpsefloat2") ||
		pev->sequence == LookupSequence("dead_grate"))
	{
		pev->movetype = MOVETYPE_NONE;
		if (FBitSet(pev->spawnflags, 0x0001))
			pev->effects = EF_INVLIGHT; // take light from the ceiling 
		ResetSequenceInfo();
	}
}

void CDeadScientist::BecomeDead(void)
{
