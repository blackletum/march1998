...
public:
	...
	static const char* pGruntStepSounds[];
};
...
const char* CHGrunt::pGruntStepSounds[] =
{
	"hgrunt/gr_step1.wav",
	"hgrunt/gr_step2.wav",
	"hgrunt/gr_step3.wav",
...
{
	if (m_iTraceCount < 3)	// if three bullets hit grunt's head at one frame and he survived, reset the trace counter
		m_iTraceCount = 0;
	//	else	
	//		ALERT( at_console, "HGrunt: m_iTraceCount >= 3!\n" );
...
//=========================================================
// CheckMeleeAttack2 - check if we got too close to keep runandgunning
//=========================================================
BOOL CHGrunt::CheckMeleeAttack2(float flDot, float flDist)
{
	CBaseMonster* pEnemy;
	...
	if (IsMoving() && flDist < 256.0 && !HasConditions(bits_COND_ENEMY_OCCLUDED))
	{
...
{
	if (!HasConditions(bits_COND_NO_AMMO_LOADED) && !HasConditions(bits_COND_ENEMY_OCCLUDED) && flDist <= 2048 && flDot >= 0.5 && NoFriendlyFire())
	{
		TraceResult	tr;
...
//=========================================================
// TraceAttack - headshot check
//=========================================================
void CHGrunt::TraceAttack(entvars_t* pevAttacker, float flDamage, Vector vecDir, TraceResult* ptr, int bitsDamageType)
{
	// prevent setting the blown up head bodygroup if the caliber isn't large enough
	if (ptr->iHitgroup == 11 || ptr->iHitgroup == HITGROUP_HEAD)
	{
		//		CBaseEntity* m_pAttacker;
		//		m_pAttacker = (CBaseEntity *)Instance(ENT(pevAttacker));

		if (flDamage >= 20 && !(bitsDamageType & DMG_BLAST || bitsDamageType & DMG_CLUB) || bitsDamageType & DMG_ENERGYBEAM)
			m_fHeadBlowup = TRUE;
		else
			m_iTraceCount++;	// wait for buckshot
...
int CHGrunt::TakeDamage(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
	...
	if (pev->health - flDamage <= 0 && m_iTraceCount >= 3)	// three bullets happen to hit grunt's head at once? blow it up!
		m_fHeadBlowup = TRUE;
	...
	if (FOkToSpeak() && (g_fGruntQuestion || RANDOM_LONG(0, 1)))
	{
		if (!g_fGruntQuestion)
		{
			...
			case 2: // statement
				...
				break;
			case 3: // alpha radio
				SENTENCEG_PlayRndSz(ENT(pev), "HG_RADIO", HGRUNT_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch);
				break;
			}
		}
		else
...
void CHGrunt::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	...
	case HGRUNT_AE_STEP:
	{
		EMIT_SOUND_DYN(ENT(pev), CHAN_ITEM, pGruntStepSounds[RANDOM_LONG(0, ARRAYSIZE(pGruntStepSounds) - 1)], 1.0, ATTN_NORM, 0, 100);
	}
	break;
	...
	case HGRUNT_AE_RELOAD:
		...
		if (RANDOM_LONG(0, 3) == 1)
			EMIT_SOUND(ENT(pev), CHAN_VOICE, "hgrunt/gr_loadtalk.wav", 1, ATTN_NORM);
...
void CHGrunt::Spawn()
{
	...
	m_afCapability = bits_CAP_SQUAD | bits_CAP_TURN_HEAD | bits_CAP_DOORS_GROUP | bits_CAP_STRAFE | bits_CAP_MELEE_ATTACK2;
	...
	if (pev->weapons == 0 || FBitSet(pev->weapons, HGRUNT_SHOTGUN))
	{
...
void CHGrunt::Precache()
{
	...
	//	PRECACHE_SOUND( "hgrunt/gr_headshot.wav" );

	for (int i = 0; i < ARRAYSIZE(pGruntStepSounds); i++)
		PRECACHE_SOUND((char*)pGruntStepSounds[i]);
...
void CHGrunt::StartTask(Task_t* pTask)
{
	...
	case TASK_STRAFE_PATH:
		...
		Forget(bits_MEMORY_INCOVER);
		if (m_fCanStrafe && m_hEnemy != NULL)
		{
			// UNDONE: extra trace to fix grunts strafing in place?
			Vector2D	vec2DirToPoint;
			Vector2D	vec2RightSide, vec2LeftSide;
			Vector2D	vec2ToEnemy;
			Vector2D	vec2Forward;
			float		flMaxDotProduct = 0.7;	// ~45 degrees
			...
			vec2LeftSide = (gpGlobals->v_right * -1).Make2D().Normalize();
			vec2ToEnemy = (m_hEnemy->pev->origin - pev->origin).Make2D();
			vec2Forward = gpGlobals->v_forward.Make2D().Normalize();

			if (DotProduct(vec2Forward, vec2ToEnemy) > flMaxDotProduct)
			{
		...
		else CSquadMonster::StartTask(pTask);
		break;
...
void CHGrunt::PainSound(void)
{
	if (gpGlobals->time > m_flNextPainTime)
	{
		...
		if (InSquad() && RANDOM_LONG(1, 3) == 1)
		{
			EMIT_SOUND(ENT(pev), CHAN_VOICE, "hgrunt/gr_cover2.wav", 1, ATTN_NORM);
			return;
		}
...
void CHGrunt::SetActivity(Activity NewActivity)
{
	...
	case ACT_RUN:
	{
		/*	Vector2D	vec2ToEnemy;
			...
			vec2ToEnemy = ( m_hEnemy->pev->origin - pev->origin ).Make2D();
			flDot = DotProduct (vec2ToEnemy , gpGlobals->v_forward.Make2D() );	*/
...
Schedule_t* CHGrunt::GetSchedule(void)
{
	...
	case MONSTERSTATE_COMBAT:
	{
			...
			if (InSquad())
			{
				...
				else
				{
					...
					if (FOkToSpeak())// && RANDOM_LONG(0,1))
					{
						...
						{
							// player
							EMIT_SOUND(ENT(pev), CHAN_VOICE, "hgrunt/gr_squadform.wav", 1, ATTN_NORM);
							JustSpoke();
							//SENTENCEG_PlayRndSz( ENT(pev), "HG_ALERT", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
							return GetScheduleOfType(SCHED_GRUNT_FOUND_ENEMY);	// it was considered anyway
						}
					...
					if (HasConditions(bits_COND_CAN_RANGE_ATTACK1) && vec2ToEnemy.Length() < 384)
					{
						return GetScheduleOfType(SCHED_GRUNT_SUPPRESS);
					}
					else
		...
		else if (HasConditions(bits_COND_LIGHT_DAMAGE) || HasConditions(bits_COND_HEAVY_DAMAGE))
		{
			// if hurt:
...
void CDeadHGrunt::BecomeDead(void)
{
...
void CHGrunt::MonsterThink(void)
{
	...
	if ((m_Activity == ACT_RUN || pev->sequence == LookupSequence("runandgun")) && m_hEnemy != NULL)
	{
		Vector2D	vec2DirToPoint;
		Vector2D	vec2ToEnemy;

		if (0 && CVAR_GET_FLOAT("sv_drawroute"))
			UTIL_ParticleEffect(EyePosition(), pev->origin + gpGlobals->v_up, 238, 4);

		vec2DirToPoint = (m_Route[0].vecLocation - pev->origin).Make2D().Normalize();
		vec2ToEnemy = (m_hEnemy->pev->origin - pev->origin).Make2D();

		//	ALERT( at_console, "HGrunt: RunAndGun check!\n");

		if (DotProduct(vec2DirToPoint, vec2ToEnemy) >= 0.25 && HasConditions(bits_COND_CAN_RANGE_ATTACK1))
		{
			//	ALERT( at_console, "HGrunt: RunAndGun!\n");
			m_fCanStrafe = FALSE;
			pev->sequence = LookupSequence("runandgun");
		}
	...
	if (0 && (m_Activity == ACT_STRAFE_LEFT || m_Activity == ACT_STRAFE_RIGHT) && HasConditions(bits_COND_CAN_RANGE_ATTACK1))	// why double check for strafefire lol
	{
		Vector2D	vec2Forward;
		Vector2D	vec2ToEnemy;
		...
		if (DotProduct(vec2Forward, vec2ToEnemy) >= 0.5 && m_cAmmoLoaded > 5)	// don't start strafefire just to dump a single bullet
		{
	...
	if ((pev->sequence == LookupSequence("strafeleft_fire") || pev->sequence == LookupSequence("straferight_fire")) && !HasConditions(bits_COND_CAN_RANGE_ATTACK1))
	{
		if (pev->sequence == LookupSequence("strafeleft_fire"))
...
//=========================================================
// MakeIdealYaw - overriden for HGrunts because the original
// implementation of strafe is buggy as hell
// 
//=========================================================
void CHGrunt::MakeIdealYaw(Vector vecTarget)
{
	Vector vecRightSide = CrossProduct((vecTarget - pev->origin).Normalize(), Vector(0, 0, 1)).Normalize();

	if (m_IdealActivity == m_movementActivity && m_movementActivity == ACT_STRAFE_LEFT)
	{
		pev->ideal_yaw = UTIL_VecToYaw(vecRightSide);
		return;
	}
...
//=========================================================
#define	COVER_CHECKS	32// how many checks are made
#define COVER_DELTA		8// distance between checks
...
BOOL CHGrunt::FindCover(Vector vecThreat, Vector vecViewOffset, float flMinDist, float flMaxDist)
{
	m_fCanStrafe = FALSE;

	return CBaseMonster::FindCover(vecThreat, vecViewOffset, flMinDist, flMaxDist);
}

BOOL CHGrunt::IsStrafing(void)
{
	if ((m_Activity == ACT_STRAFE_LEFT || m_Activity == ACT_STRAFE_RIGHT) || (pev->sequence == LookupSequence("strafefire_left")
		|| pev->sequence == LookupSequence("strafefire_right")))
		return TRUE;
