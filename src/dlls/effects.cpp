...
/*
TYPEDESCRIPTION	CTestEffect::m_SaveData[] =
{
	DEFINE_ARRAY( CTestEffect, m_pBeam, FIELD_CLASSPTR, 24 ),
	DEFINE_FIELD( CTestEffect, m_iBeam, FIELD_INTEGER ),
	DEFINE_FIELD( CTestEffect, m_flStartTime, FIELD_TIME ),
	DEFINE_ARRAY( CTestEffect, m_flBeamTime, FIELD_TIME, 24 ),
};

IMPLEMENT_SAVERESTORE( CTestEffect, CBaseDelay );*/
...
void CTestEffect::Precache(void)
{
	...
	PRECACHE_SOUND("debris/TELE1.WAV");
	PRECACHE_SOUND("debris/TELE2.WAV");
}
...
void CTestEffect::TestThink(void)
{
	...
	if (m_iBeam < 24)
	{
		...
		UTIL_TraceLine(vecSrc, vecSrc + vecDir * RANDOM_LONG(128, 256), ignore_monsters, ENT(pev), &tr);

		pbeam->PointsInit(vecSrc, tr.vecEndPos);
		//	pbeam->SetColor( 80, 100, 255 );
		//	pbeam->SetColor( 255, 180, 100 );
		pbeam->SetColor(180, 80, 70);
		pbeam->SetWidth(150);		// 100
	...
	{
		ALERT(at_console, "CTestEffect: Removing beams!\n");
		for (i = 0; i < m_iBeam; i++)
		{
...
void CTestEffect::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	// make some noise
	if (RANDOM_LONG(0, 1))
		UTIL_EmitAmbientSound(ENT(pev), pev->origin, "debris/TELE1.WAV", 1.0, ATTN_NORM, 0, 100);
	else
		UTIL_EmitAmbientSound(ENT(pev), pev->origin, "debris/TELE2.WAV", 1.0, ATTN_NORM, 0, 100);
...
class CFartFade : public CBaseEntity
{
public:
	void	Spawn(void);
	void	Precache(void);
...
LINK_ENTITY_TO_CLASS(env_telefade, CFartFade);

void CFartFade::Spawn(void)
{
	Precache();
}

void CFartFade::Precache(void)
{
	PRECACHE_MODEL("sprites/lgtning.spr");
	PRECACHE_SOUND("custom/telefart.wav");
}

void CFartFade::TestThink(void)
{
	if (!FStringNull(pev->message))
	{
		pentTarget = FIND_ENTITY_BY_TARGETNAME(pentTarget, STRING(pev->message));

		if (!FNullEnt(pentTarget))
		{
			CBaseMonster* pEnt = (CBaseMonster*)CBaseEntity::Instance(pentTarget);
			pEnt->FadeTeleMonster();

			CBaseEntity* pEffect1 = Create("test_effect", ((pEnt->pev->absmax + pEnt->pev->absmin) * 0.5), pev->angles);
			pEffect1->Use(this, this, USE_ON, 1);

			EMIT_SOUND(ENT(pev), CHAN_AUTO, "custom/telefart.wav", 1, ATTN_NORM);
			SetThink(&CBaseEntity::SUB_Remove);
		}
		else
		{
			ALERT(at_console, "Could not find monster\n");
			SetThink(&CBaseEntity::SUB_Remove);
		}
	}
	else
	{
		ALERT(at_console, "Message is NULL\n");
		SetThink(&CBaseEntity::SUB_Remove);
	}
}

void CFartFade::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	SetThink(&CFartFade::TestThink);
	pev->nextthink = gpGlobals->time + 0.1;
}
...
void CBlood::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	if ( pev->spawnflags & SF_BLOOD_STREAM )
		...
		UTIL_Blood(BloodPosition(pActivator), Direction(), (Color() == BLOOD_COLOR_RED) ? 70 : Color(), BloodAmount());
