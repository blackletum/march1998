...
{
	int animDesired = LookupSequence("walk");
	int animrandom1 = LookupSequence("walk_shake");
	int animrandom2 = LookupSequence("walk_spasm");
	int animrandom3 = LookupSequence("walk_stumble");

	//random stumble anim
	if (m_fSequenceFinished && pev->sequence == animDesired)
	{
		wobbo = RANDOM_LONG(0, 10);
		if (wobbo == 5)
		{
			int rando = (RANDOM_LONG(1, 3));
			if (rando == 1)
				anim = animrandom1;
			else if (rando == 2)
				anim = animrandom2;
	...
	//back to normal walk
	if (m_fSequenceFinished && pev->sequence == anim)
	{
		pev->frame = 4;
		pev->sequence = animDesired;
		ALERT(at_console, "moogly\n");
		ResetSequenceInfo();
		SetYawSpeed();
	}
