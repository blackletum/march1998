...
void CPython::Precache( void )
{
	...
	PRECACHE_SOUND ("weapons/357_shoot.wav");
}
...
*/
	...
	EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/357_shoot.wav", RANDOM_FLOAT(0.8, 0.9), ATTN_NORM);
	...
	if (m_flSoundDelay != 0 && m_flSoundDelay <= gpGlobals->time)
	{
		...
		for (int i = 0; i < 6; i++)
		{
			EjectBrass ( m_pPlayer->pev->origin, 
						...
						m_pPlayer->pev->angles.y, TE_BOUNCE_SHELL, TE_BOUNCE_SHELL);
		}
