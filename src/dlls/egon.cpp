...
void CEgon::SecondaryAttack( void )
{
	...
	else if ( m_fireMode == FIRE_WIDE )
	{
		...
		m_flNextPrimaryAttack = m_flNextSecondaryAttack = gpGlobals->time + EGON_SWITCH_NARROW_TIME;
	}
	else
	{
		...
		m_flNextPrimaryAttack = m_flNextSecondaryAttack = gpGlobals->time + EGON_SWITCH_WIDE_TIME;
	}
}
...
void CEgon::UpdateEffect(const Vector& startPoint, const Vector& endPoint, float timeBlend)
{
	if (CVAR_GET_FLOAT("cl_m98_egonbeamclassic") != 1)
	{
		...
		WRITE_COORD(startPoint[1]);
		WRITE_COORD(startPoint[2]);
...
{
	if (CVAR_GET_FLOAT("cl_m98_egonbeamclassic") != 0)
	{
		if (m_pBeam)
		{
			UTIL_Remove(m_pBeam);
