...
{
	if (!(bitsDamageType & (DMG_CLUB | DMG_SLASH | DMG_BULLET | DMG_ENERGYBEAM | DMG_BLAST)))
		return 0;
...
int CTripmine::AddToPlayer(CBasePlayer* pPlayer)
{
	if (CBasePlayerWeapon::AddToPlayer(pPlayer))
	{
		...
		WRITE_BYTE(m_iId);
		MESSAGE_END();
		m_pPlayer->SetSuitUpdate("!HEV_TRIPMINE", FALSE, SUIT_NEXT_IN_30MIN);
		return TRUE;
	}
	return FALSE;
}
