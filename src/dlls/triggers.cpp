...
#define SF_TRIGGER_PUSH_NO_MONSTERS	64// ignore monsters
...
void CTriggerMonsterJump::Touch(CBaseEntity* pOther)
{
	...
	ALERT(at_console, "Tossed %s with a speed of%f and to a height of %f\n", STRING(pOther->pev->classname), pev->speed, m_flHeight);
}
...
void CChangeLevel::ChangeLevelNow(CBaseEntity* pActivator)
{
	...
	// hack to force StartMonster() for monsters that were spawned by portal so they wont be standing still when player will return to the previous map
	m_skipPortal = TRUE;
...
int CChangeLevel::InTransitionVolume(CBaseEntity* pEntity, char* pVolumeName)
{
	...
	{
		ALERT(at_console, "Forced transition for %s\n", STRING(pEntity->pev->classname));
		return 1;
	}
...
public:
	...
	// Vasia: Imitate SOLID_NOT without actually setting it. 
	// Part of the ladder fix. ( Source 1 improvement )
//	 bool ShouldCollide( CBaseEntity *pOther ) { return false; }
};
...
void CTriggerPush::Touch(CBaseEntity* pOther)
{
	...
	if (FBitSet(pevToucher->flags, FL_MONSTER) && !pOther->IsPlayer() && FBitSet(pev->spawnflags, SF_TRIGGER_PUSH_NO_MONSTERS))
	{
		ALERT(at_console, "trigger_push skips %s\n", STRING(pOther->pev->classname));
		return;
	}
...
class CSizeModifier : public CBaseEntity
{
public:
	void KeyValue(KeyValueData* pkvd);
	void Spawn(void);
...
LINK_ENTITY_TO_CLASS(env_size, CSizeModifier);


void CSizeModifier::KeyValue(KeyValueData* pkvd)
{
	if (FStrEq(pkvd->szKeyName, "_mins"))
	{
		int x, y, z, m;
		m = sscanf(pkvd->szValue, "%d %d %d\n", &x, &y, &z);
		...
		else ALERT(at_console, "Failed to read mins for env_size!\n");

		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "_maxs"))
	{
		int x, y, z, m;
		m = sscanf(pkvd->szValue, "%d %d %d\n", &x, &y, &z);
...
void CSizeModifier::Spawn(void)
{
	pev->solid = SOLID_NOT;
}

void  CSizeModifier::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	if (!vecMin || !vecMax)
		return;
	...
	if (!FStringNull(pev->target))
	{
		...
		UTIL_SetSize(pTarget->pev, vecMin, vecMax);

		ALERT(at_console, "Set size for %s to %f, %f, %f and %f, %f, %f\n",
			STRING(pTarget->pev->classname), vecMin.x, vecMin.y, vecMin.z, vecMax.x, vecMax.y, vecMax.z);
		//	ALERT( at_console, "%f, %f, %f, %f, %f, %f"\n, pTarget->pev->mins.x, pTarget->pev->mins.y, pTarget->pev->mins.z, pTarget->pev->maxs.x, pTarget->pev->maxs.y, pTarget->pev->maxs.z );
	}
}
...
void CTeleDeath::DeathTouch(CBaseEntity* pOther)
{
	...
	{
		ALERT(at_aiconsole, "TELEFRAGGED %s\n", STRING(pOther->pev->classname));
		pOther->TakeDamage(pOther->pev, pOwner->pev, 1000, DMG_GENERIC | DMG_ALWAYSGIB);
	}
}

class CTargetCommand : public CBaseTrigger
{
public:
	void KeyValue(KeyValueData* pkvd);
	void Spawn(void);
...
LINK_ENTITY_TO_CLASS(trigger_command, CTargetCommand);
LINK_ENTITY_TO_CLASS(target_command, CTargetCommand);

TYPEDESCRIPTION	CTargetCommand::m_SaveData[] =
{
	DEFINE_FIELD(CTargetCommand, m_command, FIELD_STRING),
};

IMPLEMENT_SAVERESTORE(CTargetCommand, CBaseDelay);
...
void CTargetCommand::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	edict_t* pClient;
...
class CTriggerShower : public CBaseTrigger
{
public:
	void Spawn(void);
	void KeyValue(KeyValueData* pkvd);
	void EXPORT HealTouch(CBaseEntity* pOther);
...
LINK_ENTITY_TO_CLASS(trigger_shower, CTriggerShower);

TYPEDESCRIPTION	CTriggerShower::m_SaveData[] =
{
	DEFINE_FIELD(CTriggerShower, m_iDamageType, FIELD_INTEGER),
};

IMPLEMENT_SAVERESTORE(CTriggerShower, CBaseDelay);

void CTriggerShower::KeyValue(KeyValueData* pkvd)
{
	if (FStrEq(pkvd->szKeyName, "dmgtype"))
	{
...
void CTriggerShower::Spawn(void)
{
	InitTrigger();
	SetTouch(&CTriggerShower::HealTouch);

	if (!FStringNull(pev->targetname))
	{
		SetUse(&CTriggerShower::ToggleUse);
		pev->solid = SOLID_NOT;
	}
	else
	{
...
void CTriggerShower::HealTouch(CBaseEntity* pOther)
{
	if (!pOther->IsPlayer())
		return;
	...
	pPlayer = (CBasePlayer*)pOther->Instance(pOther->edict());
	...
	for (int i = 0; i < CDMG_TIMEBASED; i++)
	{
		...
		if (pPlayer->m_rgbTimeBasedDamage[i])
		{
			pPlayer->m_rgbTimeBasedDamage[i] = 0;
			pPlayer->m_bitsDamageType &= ~(DMG_PARALYZE << i);
		}
	}
...
#define SF_TRIGGER_UPDATE_TARGETONCE	1
#define	SF_TRIGGER_UPDATE_START_OFF		2

class CTriggerSuitUpdate : public CBaseTrigger
{
public:
	void Spawn(void);
	void KeyValue(KeyValueData* pkvd);
...
TYPEDESCRIPTION	CTriggerSuitUpdate::m_SaveData[] =
{
	DEFINE_FIELD(CTriggerSuitUpdate, m_iszSentence, FIELD_STRING),
	DEFINE_FIELD(CTriggerSuitUpdate, m_iNoRepeat, FIELD_INTEGER),
};


IMPLEMENT_SAVERESTORE(CTriggerSuitUpdate, CBaseDelay);

LINK_ENTITY_TO_CLASS(trigger_suitupdate, CTriggerSuitUpdate);

void CTriggerSuitUpdate::KeyValue(KeyValueData* pkvd)
{
	...
	else if (FStrEq(pkvd->szKeyName, "repeat"))
	{
		int kv = atoi(pkvd->szValue);
...
void CTriggerSuitUpdate::Spawn(void)
{
	if (m_flWait <= 0)
		m_flWait = 0.2;
	...
	if (!FStringNull(pev->targetname))
	{
		SetUse(&CTriggerSuitUpdate::ToggleUse);
	}
	else
	{
		SetUse(NULL);
	...
	if (pev->spawnflags & SF_TRIGGER_UPDATE_START_OFF)
		pev->solid = SOLID_NOT;
