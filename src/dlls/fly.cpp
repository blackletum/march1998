...
//=========================================================
// ENV_FLY - Spawns a swarm of flies, for maps and also used to generate the rare swarm on corpses.
//=========================================================
class CFlySpawner : public CBaseEntity
{
public:
	void	Spawn(void);
	static CFly* FlyCreate(const Vector& origin, const Vector& angle, edict_t* pentOwner);
};

LINK_ENTITY_TO_CLASS(env_fly, CFlySpawner);

void CFlySpawner::Spawn(void)
{
	//Precache();
	pev->solid = SOLID_NOT;

	CFlySpawner::FlyCreate(pev->origin, pev->angles, edict());

}

CFly* CFlySpawner::FlyCreate(const Vector& origin, const Vector& angle, edict_t* pentOwner)
{
	int	i;
	CBaseEntity* pFly = nullptr;
	CFly* Flyguy = GetClassPtr((CFly*)NULL);
	Flyguy->pev->classname = MAKE_STRING("env_fly");
	for (i = 0; i < 5; i++)
	{
		//Flyguy->Spawn();
		pFly = CBaseEntity::Create("monster_fly", origin, angle, pentOwner);
	}
	return Flyguy;
}