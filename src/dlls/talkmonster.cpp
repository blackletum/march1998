...
void CTalkMonster :: RunTask( Task_t *pTask )
{
	...
	case TASK_TLK_LOOK_AT_CLIENT:
			 ...
			 !IsTalking()								)
		{
			...
			if ( pPlayer )
			{
				IdleHeadTurn( pPlayer->v.origin + pPlayer->v.view_ofs);
			}
		}
		else
		{
	...
	case TASK_TLK_EYECONTACT:
	{
		...
		{
			if (m_hTalkTarget != pPlayer) //headcrab - hack to fix scientists not properly looking at the player
				// ALERT( at_console, "waiting %f\n", m_flStopTalkTime - gpGlobals->time );
				IdleHeadTurn(m_hTalkTarget->pev->origin + pev->view_ofs);
			else
			{
				prealPlayer = g_engfuncs.pfnPEntityOfEntIndex(1);
				IdleHeadTurn(prealPlayer->v.origin + prealPlayer->v.view_ofs);
			}
		}
		else
		{
...
void CTalkMonster :: IdleHeadTurn( Vector &vecFriend )
{
	...
	if (m_afCapability & bits_CAP_TURN_HEAD)
	{
		...
		Vector yaw2 = UTIL_VecToAngles((vecFriend ) - (pev->origin + pev->view_ofs)); //Headcrab - readding second head controller
		float zangle = -yaw2.x;
		//ALERT(at_console, "yaw : %f\n", zangle);
		//ALERT(at_console, "yaw2 : %f\n", yaw);
		if (yaw > 180) yaw -= 360;
		if (yaw < -180) yaw += 360;
		if (zangle < -300) zangle += 370;
		if (zangle == -270) zangle += 270; //Headcrab - HACK scientists look at their origin sometimes to stop looking at the player, but it makes them look at their feet
		//if (yaw2 < -180) yaw2 += 360;
		if (zangle < -43) yaw = yaw/2; //Headcrab - HACK some jank code to make them looking up and to the side look more natural
	
		// turn towards vector
		SetBoneController( 0, yaw );
		SetBoneController(1, zangle);
	}
}
