...
LINK_ENTITY_TO_CLASS(func_glass, CBreakable);
...
void CBreakable::BreakTouch( CBaseEntity *pOther )
{
	...
	if (( FBitSet ( pev->spawnflags, SF_BREAK_TOUCH ) ) || (FClassnameIs(pev, "func_glass")))
	{// can be broken when run into 
		...
		if (flDamage >= pev->health)
		{
			...
			//pOther->TakeDamage( pev, pev, flDamage/4, DMG_SLASH ); //magic nipples - no damage because poop
		}
	}
...
void CBreakable::Die( void )
{
	...
	case matMetal:
		...
		cFlag = BREAK_SMOKE; //magic nipples - smoke when broken | BREAK_METAL;
		break;
