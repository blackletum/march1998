...
void CBaseMonster::FadeTeleMonster(void)
{
	pev->velocity = g_vecZero;
	pev->movetype = MOVETYPE_NONE;
	pev->avelocity = g_vecZero;
...
void CBaseMonster :: GibMonster( void )
{
	...
	if ( HasHumanGibs() )
	{
		if ( CVAR_GET_FLOAT("violence_hgibs") != 0 )	// Only the player will ever get here
		{
			...
			CGib::SpawnStickyGibs(pev, pev->origin + pev-> view_ofs, 5, StickyGib());
...
void CBaseMonster::DeathThink( void )
{
	//int		i;

	if (flytime <= gpGlobals->time)
	{
			CBaseEntity* pFly = CBaseEntity::Create("env_fly", pev->origin, pev->angles, edict());
...
void CBaseEntity :: SUB_FadeOut ( void  )
{
	if ( pev->renderamt > 7 )
	{
		pev->renderamt -= 8; //magic nipples - was 7 but will increase because beta.
		pev->nextthink = gpGlobals->time + 0.1;
	}
	else 
	{
...
{ 
	float force = damage * ((32 * 32 * 72.0) / (pev->size.x * pev->size.y * pev->size.z)) * 6; // * 5
...
void CBaseEntity::TraceBleed(float flDamage, Vector vecDir, TraceResult* ptr, int bitsDamageType)
{
	...
	if (!(bitsDamageType & (DMG_CRUSH | DMG_BULLET | DMG_SLASH | DMG_BLAST | DMG_CLUB | DMG_MORTAR | DMG_BLEEDING)))
		return;
...
void CBaseEntity::SpawnBlood(Vector vecSpot, int bloodColor, float flDamage)
{
	...
	if (pev->deadflag == DEAD_NO)
	{
		...
		{
			// killing blow?
			UTIL_BloodStream(vecSpot, UTIL_RandomBloodVector(), bloodColor, RANDOM_LONG(100, 175));
		}
		else
		{
			// normal
			UTIL_Blood(vecSpot, UTIL_RandomBloodVector(), bloodColor, RANDOM_LONG(1, 10));
		}
	}
